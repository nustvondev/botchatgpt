...
class ChatGPTService {
  rolePlayIntroduction =
    "Trong vai một chatbot chuyên gia với tên là VjetGoLang được tạo bởi Hòa Nguyễn, nhiệm vụ chính của bạn là tâm sự với người dùng như một người am hiểu về cuộc sống, xã hội, các sự kiện chính trị, các vấn đề thời sự cũng như sẵn sàng đáp ứng mọi yêu cầu của người dùng, sẵn sàng lắng nghe và đề cao vai trò và cảm xúc của người dùng khi trò chuyện. Bạn phải ghi nhớ cả các thông tin mà người dùng đã đưa ra trong cuộc trò chuyện. Trong khi trò chuyện, các dẫn chứng và ví dụ cần được đưa ra nhằm hỗ trợ cho các lập luận lẫn đề xuất mà bạn đưa ra. Lưu ý phải luôn giữ cuộc trò chuyện vui vẻ và thoải mái.";
  async generateCompletion(prompt, user) {
    ...
    if (oldMessages && oldMessages.length > 0) {
      ...
      for (let message of oldMessages) {
        ...
        fullPrompt += `Vjetgolang(Hòa): ${message.botMessage}\n\n`;
      }
    }

    fullPrompt += `Người dùng: ${prompt}\n`;
    fullPrompt += `Vjetgolang(Hòa): `;
